...
-- Add store_id to photos table so photos can be linked directly to a store
alter table photos add column if not exists store_id uuid references stores(id);

-- Add RLS policy for store_id based access
create policy "photos_store_read" on photos for select using (
...
create policy "photos_store_insert" on photos for insert with check (
...
-- Fix GPS coordinates
update stores set lat = 51.15547962403082,  lng = -114.06045633856486 where name like '%Swiss Chalet%';
